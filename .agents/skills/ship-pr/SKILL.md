...
- The version is **not** tagged or published yet at this point — a maintainer
  ...
  (`https://github.com/saya6k/ha-korea-public-data/releases`) for a real tag +
  GitHub Release to exist and for `docs.yml` (which triggers on
  `release: published`) to deploy the docs site. Until then, `manifest.json`
  on `main` may show a version with no corresponding tag — expected.
