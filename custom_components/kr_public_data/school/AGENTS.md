...
Legacy pre-subentry entries (all fields flat in `entry.data`) still load through a fallback path in `__init__.py`/`sensor.py`/`calendar.py` — don't remove it. `llm/school_tool.py` picks which school's coordinator to use via an optional `school` tool parameter resolved against `store["school_subs"]` (see root `AGENTS.md` hard rule 7).
